...
## 下游：專利佈局策略（patent-deployment）

本技能產出的圖表可直接作為 [patent-deployment](https://github.com/jack-lee2022/patent-deployment) 技能的決策依據：

| 本技能圖表輸出 | 供 patent-deployment 決策的洞察 |
| :--- | :--- |
| **技術功效矩陣 Blue Ocean 格** | 識別可申請的技術空白，作為堡壘式或圍牆式的卡位目標 |
| **競爭者雷達圖** | 找出對手在各 IPC 維度的弱項，作為斷路式布局的瓶頸點 |
| **技術演進時間軸** | 確認各技術分支的活躍期與衰退期，決定地毯式時機或防禦式放棄 |
| **申請人×年度熱圖** | 偵測競爭者突然沉寂（可能轉向），提示需追加針對性搜尋 |
